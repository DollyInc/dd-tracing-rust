...
impl Event {
    ...
    pub fn log(&self, level: &tracing::Level, logger: &slog::Logger) {
        ...
        match *level {
            tracing::Level::ERROR => {
                error!(logger, ""; o!(kv, "status" => "error", "message" => message))
            }
            tracing::Level::WARN => {
                warn!(logger, ""; o!(kv, "status" => "warn", "message" => message))
            }
            tracing::Level::INFO => {
                info!(logger, ""; o!(kv, "status" => "info", "message" => message))
            }
            tracing::Level::DEBUG => {
                debug!(logger, ""; o!(kv, "status" => "debug", "message" => message))
            }
            tracing::Level::TRACE => {
                trace!(logger, ""; o!(kv, "status" => "trace", "message" => message))
            }
        }
    }
}
