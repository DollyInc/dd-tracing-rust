use std::{collections::HashMap, time::{Duration, SystemTime}, fmt::Debug};
...
impl Span {
  pub fn new(parent: Option<tracing::Id>, trace_id: u64, attrs: &tracing::span::Attributes) -> Self {
    let metadata = attrs.metadata();
    let name = metadata.name();
    let resource = metadata.target();
    let mut span = Span { parent, trace_id, handlers: 1, data: SpanData::new(name, resource) };
  ...
  pub fn set_duration(&mut self) {
    let start = self.data.start.unwrap_or(SystemTime::now());
  ...
  pub fn into(id: tracing::Id, span: Span) -> datadog_apm::Span {
    ...
    let start = data.start.unwrap_or(SystemTime::now());
    let duration = data.duration.unwrap_or(
      SystemTime::now().duration_since(start).unwrap_or(Duration::from_nanos(0))
    );
    datadog_apm::Span {
      id: id.into_u64(),
...
impl tracing::field::Visit for Span {
  ...
  fn record_debug(&mut self, field: &tracing::field::Field, value: &dyn Debug) {
    let name = field.name();
    if name != "callsite" {
      self.data.tags.insert(name.to_string(), format!("{:#?}", value));
    }
  }
}