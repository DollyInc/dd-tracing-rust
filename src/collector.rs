...
impl tracing::Subscriber for Collector {
    fn enabled(&self, metadata: &tracing::Metadata<'_>) -> bool {
        *metadata.level() <= self.level && metadata.target().starts_with(&self.prefix)
    }
    fn new_span(&self, span: &tracing::span::Attributes<'_>) -> tracing::Id {
        ...
        let trace_id = parent
            .as_ref()
            .map(|parent_id| spans.get(parent_id).map(|parent_span| parent_span.trace_id))
            .flatten()
            .unwrap_or_else(|| self.get_next_trace_id());
        let mut traces = self.traces.lock().unwrap();
        let trace_spans = traces.entry(trace_id).or_insert_with(Vec::new);
    ...
    fn event(&self, event: &tracing::Event<'_>) {
        ...
        let mut ev = Event::new(
            metadata.target(),
            parent_ref.map(|p| p.name()),
            parent_id.map(|p| p.into_u64()),
            parent_ref.map(|p| p.trace_id),
        );
        event.record(&mut ev);
    ...
    fn try_close(&self, span_id: tracing::Id) -> bool {
        ...
        if let Some(span) = spans.get_mut(&span_id) {
            ...
            if span.is_closed() {
                span.set_duration();
                if span.parent == None {
                    // if closing a parent, its trace is done
